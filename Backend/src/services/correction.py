...
class WordCorrectionService:
    ...
    def _clean_quotes_from_text(self, text: str) -> str:
        """Remove unwanted quotes from corrected text"""
        if not text:
            return text
    ...
    def correct_text(self, transcribed_text: str, context: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            corrected_text = self._clean_quotes_from_text(corrected_text)
...
❌ สิ่งที่ห้ามทำ:
- ห้ามเพิ่มข้อมูลใหม่ที่ผู้พูดไม่ได้พูด
- ห้ามลบหรือตัดข้อมูลที่ผู้พูดพูด
- ห้ามเปลี่ยนโครงสร้างประโยคมากเกินไป
- ห้ามเปลี่ยนความหมายหรือเจตนาของผู้พูด
- ห้ามทำให้ภาษาเป็นทางการจนเกินไป
- ห้ามใส่เครื่องหมายคำพูด (quotes) "" หรือ '' ในคำตอบ

📋 รูปแบบการตอบ:
- ตอบเฉพาะข้อความที่แก้ไขแล้ว โดยไม่มีเครื่องหมายคำพูด
- ไม่ต้องอธิบายหรือแสดงความคิดเห็น
- ถ้าข้อความถูกต้องแล้ว ให้ส่งกลับข้อความเดิมทุกอย่าง
...
⚠️ สำคัญมาก:
- ตอบเฉพาะข้อความที่แก้ไขแล้วเท่านั้น
- ห้ามใช้เครื่องหมายคำพูด "" หรือ '' ในคำตอบ
- ถ้าข้อความถูกต้องแล้ว ให้ตอบข้อความเดิมทุกประการ"""
    ...
    def _build_correction_prompt(self, text: str, context: str) -> str:
        ...
        prompt += "\n\n⚠️ สำคัญ: ตอบเฉพาะข้อความที่แก้ไขแล้ว โดยไม่มีเครื่องหมายคำพูด"
